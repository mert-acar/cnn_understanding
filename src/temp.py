...
if __name__ == "__main__":
    ...
    for l_num in range(2, 4):
        ...
        filenames = sorted(glob(os.path.join(experiment_path, varname,
                                             "*.mat")),
                           key=extract_epoch)
        ...
        for fname in filenames:
            ...
            print(
                "+ Num_clusters:",
                len(set(clusters)) - 1,
                f"| Noisy Points: {100 * sum(clusters == -1) / len(clusters):.2f}"
            )
            df["n_clusters"].append(len(set(clusters)) - 1)
            df["n_noisy_samples"].append(100 * sum(clusters == -1) /
                                         len(clusters))
            df["homogeneity_score"].append(
                metrics.homogeneity_score(labels, clusters))
            df["completeness_score"].append(
                metrics.completeness_score(labels, clusters))
            df["silhouette_score"].append(
                metrics.silhouette_score(recon, clusters))
